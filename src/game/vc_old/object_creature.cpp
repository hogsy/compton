...
void Creature::Simulate()
{
	...
	for ( double &emotion : emotions_ )
	{// ensure threshold is maintained...
		if ( emotion < 0 )
		{
			emotion = 0;
		}
	...
	{// movement
		if ( grabbed_object_ == nullptr && look_object_ != nullptr )
		{
			if ( !look_object_->is_grabbed )
			{
				...
				else if ( !PickupObject( look_object_ ) )
...
void Creature::ThrowObject()
{
  ...
  if(relative_mousepos(x) < 0 || relative_mousepos(x) > 64) {
	...
	al_play_sample( game.sample_throw, 1, ALLEGRO_AUDIO_PAN_NONE, 1, ALLEGRO_PLAYMODE_ONCE, nullptr );
}
...
void Creature::SetDirective( unsigned int dir )
{
	switch ( directive_ )
	{
		...
		{
			if ( occupation_ == OCU_SLEEPING )
			{// only do this if we're actually sleeping!
