...
FILE *current_print_file = nullptr;

LSymbol *LSymbol::root = nullptr;
...
{
	size_t size = PL_MAX( sizeof( LFixedPoint ), sizeof( LRedirect ) );
...
{
	size_t size = PL_MAX( sizeof( LObjectVar ), sizeof( LRedirect ) );
...
LPointer *LPointer::Create( void *addr )
{
	if ( addr == nullptr )
		return nullptr;
	size_t size = PL_MAX( sizeof( LPointer ), sizeof( LRedirect ) );
...
{
	size_t size = PL_MAX( sizeof( LChar ), sizeof( LRedirect ) );
...
{
	size_t size = PL_MAX( sizeof( LString ) + length - 1, sizeof( LRedirect ) );
...
LUserFunction *new_lisp_user_function( LList *arg_list, LList *block_list )
{
	...
	size_t size = PL_MAX( sizeof( LUserFunction ), sizeof( LRedirect ) );
...
{
	size_t size = PL_MAX( sizeof( LSysFunction ), sizeof( LRedirect ) );
...
{
	size_t size = PL_MAX( sizeof( LSymbol ), sizeof( LRedirect ) );
...
{
	size_t size = PL_MAX( sizeof( LNumber ), sizeof( LRedirect ) );
...
LList *LList::Create()
{
	size_t size = PL_MAX( sizeof( LList ), sizeof( LRedirect ) );
...
void *lisp_equal( void *n1, void *n2 )
{
	...
	switch ( t1 )
	{
		...
		case L_CONS_CELL:
			...
			if ( n1 || n2 )
				return nullptr;// if one is longer than the other
			return true_symbol;
		default:
...
LObject *LObject::Compile( char const *&code )
{
	...
	if ( !read_ltoken( code, n ) )
		lerror( nullptr, "unexpected end of program" );
	...
	{
		LObject *cs = LList::Create(), *c2 = nullptr, *tmp;
		PtrRef r1( cs ), r2( c2 );
	...
	{
		void *first = nullptr, *cur = nullptr, *last = nullptr;
	...
	switch ( item_type( this ) )
	{
		...
		case L_CHARACTER:
		{
			if ( current_print_file != nullptr )
			{
				...
				fwrite( &ch, sizeof( char ), 1, current_print_file );
				break;
			}
...
LObject *LSymbol::EvalFunction( void *arg_list )
{
	...
	switch ( t )
	{
		...
		case L_C_BOOL:
		{
			LList *first = nullptr, *cur = nullptr;
...
void *mapcar( void *arg_list )
{
	...
	LList *na_list = nullptr, *return_list = nullptr, *last_return = nullptr;

	do
	{
		na_list = nullptr;// create a cons list with all of the parameters for the function

		LList *first = nullptr;// save the start of the list
...
void *backquote_eval( void *args )
{
	...
	{
		void *first = nullptr, *last = nullptr, *cur = nullptr, *tmp;
	...
	return nullptr;// for stupid compiler messages
}
...
LObject *LSysFunction::EvalFunction( LList *arg_list )
{
	...
	switch ( fun_number )
	{
		...
		case SYS_FUNC_SETF:
		{
			LObject *set_to = CAR( CDR( arg_list ) )->Eval(), *i = nullptr;
    ...
    case SYS_FUNC_WRITE_PROFILE:
    {
		...
		case SYS_FUNC_SEARCH:
		{
			...
			ret = find ? LNumber::Create( find - haystack ) : nullptr;
			break;
		}
		case SYS_FUNC_ELT:
		{
		...
		case SYS_FUNC_LISTP:
		{
			...
			ret = ( t == L_CONS_CELL ) ? true_symbol : nullptr;
			break;
		}
		case SYS_FUNC_NUMBERP:
		{
			LObject *tmp = CAR( arg_list )->Eval();
			ltype t = item_type( tmp );
			ret = ( t == L_NUMBER || t == L_FIXED_POINT ) ? true_symbol : nullptr;
			break;
		}
		case SYS_FUNC_DO:
		{
			...
			{
				i = CAR( CAR( CDR( arg_list ) ) )->Eval() != nullptr;
				if ( !i )
				{
		...
		{
			LObject *tmp = CAR( arg_list )->Eval();
			ret = ( item_type( tmp ) == L_SYMBOL ) ? true_symbol : nullptr;
			break;
		}
		...
		case 99:
		{
			LObject *r = nullptr, *rstart = nullptr;
...
LObject *LObject::Eval()
{
	...
	maxevaldepth = PL_MAX( maxevaldepth, ++evaldepth );
	...
	if ( trace_level )
	{
		...
		{
			dprintf( "%d (%zu, %zu, %zu) TRACE : ", trace_level,
...
void Lisp::Init()
{
	...
	clisp_init();
	LSpace::Current = &LSpace::Tmp;
	dprintf( "Lisp: %zu symbols defined, %lu system functions, "
