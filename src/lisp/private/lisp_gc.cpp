...
static void ***reg_ptr_list = nullptr;
...
{
	size_t s = x->m_len;
	LArray *a = LArray::Create( s, nullptr );
...
{
	LList *prev = nullptr, *first = nullptr;
...
LObject *Lisp::CollectObject( LObject *x )
{
	...
	maxgcdepth = PL_MAX( maxgcdepth, ++gcdepth );
	...
	else if ( ( uint8_t * ) x < collected_start || ( uint8_t * ) x >= collected_end )
	{
		...
		for ( LObject *cell = nullptr; x; cell = x, x = CDR( x ) )
		{
