...
#include "render_bitmap_font.h"

bool BitmapFont::LoadFromFile( uint8_t cw, uint8_t ch, const char *path )
{
...
void BitmapFont::DrawCharacter( int x, int y, char c, const hei::Colour &colour ) const
{
	if ( c < 0 )
	{
		return;
...
void BitmapFont::DrawString( int *x, int *y, const char *sentence, const hei::Colour &colour, bool shadow ) const
{
	if ( shadow )
	{
		int xx = *x + 1, yy = *y + 1;
