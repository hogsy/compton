...
ScriptParser::ScriptParser( const char *path )
{
 ...
 */
const char *ScriptParser::GetToken( char *buffer, size_t bufSize )
{
...
bool ScriptParser::IsEndOfFile()
{
	return *bufPos == '\0';
}

void ScriptParser::SkipSpaces()
{
	while ( *bufPos == ' ' )
	{
		AdvanceBufferPosition();
 ...
 */
const char *ScriptParser::AdvanceBufferPosition()
{
	linePos++;
	return bufPos++;
}
